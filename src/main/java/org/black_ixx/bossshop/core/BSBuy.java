...
public class BSBuy {
	...
	public boolean alreadyBought(Player p){
		...
		if (buyT == BSBuyType.Permission){
			...
			for (String s : (List<String>)reward){ //Loop durch Perms
				...
				if (!p.hasPermission(s)){ //Sobald ein Spieler eine Perm nicht hat wird false returned
					return false;
				}				
			}
	...
	private boolean hasPoints(Player p, Integer points){
		if (ClassManager.manager.getPointsManager().getPoints(p.getName())<points){
	...
	private String takePoints(Player p, Integer d){
		return ""+ClassManager.manager.getPointsManager().takePoints(p.getName(), d);
	}
	...
	private void giveRewardPoints(Player p, Integer points){
		ClassManager.manager.getPointsManager().givePoints(p.getName(), points);
	}
	...
	private void giveRewardShop(Player p, String shopName){
		if(shopName==null||shopName==""||shopName.length()<1){ //Shop == "" oder '' -> Close Inventory
			p.closeInventory();
			return;
		}

			...
			if (args.length>0){
				...
				if (args[0].equalsIgnoreCase("check")){
					...
					if (sender.hasPermission("BossShop.check")){
						...
						String date = /*ClassManager.manager.getBugFinder().getDate();*/ formatter.format(new Date());
						...
						//						}else{
						//
						//							for (int b  : ClassManager.manager.getVillagerHandler().getVillagers().keySet()){
						//								Villager v = ClassManager.manager.getVillagerHandler().getVillagers().get(b);
						...
						//										sho = ((ShopConfigHandler)v.getMetadata("BossShop").get(0).value()).getShopName();
						//										String txt=	ChatColor.YELLOW+" - "+b+" | Name "+n+" | Shop "+sho+" | Profession id "+v.getProfession().getId();
						//										check(txt,l,sender);
						//									}
						//								}
						//
